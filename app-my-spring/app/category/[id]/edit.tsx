...
import {useGetCategoryQuery, useUpdateCategoryMutation} from "@/services/categoriesService";
...
export default function EditCategoryScreen() {
    ...
    useEffect(() => {
        if (category) { // Коли категорія завантажена, оновлюємо стейт
            setForm({
                id: category.id,
                name: category.name,
                description: category.description
            });
            setImage(`${IMAGES_URL}/400_${category.image}`);
        }
    }, [category]);
    ...
    const handleSubmit = async () => {
        console.log("Редагування категорії:", form);
        try {
            const file = image && !image.startsWith(IMAGES_URL)
                ? await getFileFromUriAsync(image)
                : null;
            ...
            console.log("Category updating is problem:", err);
        }
    };
